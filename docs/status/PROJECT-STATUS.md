...
- **프리셋 4개 + 겹침 게이트** (D53) — `새로 ▾` 로 배치안을 꺼낸다. 겹침 검사를 넣자마자
  **옛 배치안에서 진짜 결함 5건**이 나왔다(컨베이어가 방폭 격벽 관통 · `workstation` 원점 122mm 치우침)

**다음 세션 첫 작업** — 팔레트를 손으로 눌러 본다(드래그·폰). 그 뒤 P2:
FR5·터틀봇을 팔레트에. **터틀봇 GLB 가 저장소에 없다** — ROBOTIS 공식 URDF 에서 변환하고,
FR5 다중 배치는 `layout.arm` 단일 필드라 `SHARED-CORE.md` 가 먼저다 (하드 룰 1).
배치 화면에 **로봇 팔을 그릴지**도 정해야 한다 — `mountArm` 훅은 이미 있고 6MB 가 값이다.
