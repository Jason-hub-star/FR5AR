...
**정합을 손에서 센서로 넘겼다 (2026-08-07)** — 최대 오차는 캘리브레이션이 아니라 손으로
돌리던 `↺↻` 15° 였다(12m 에서 157cm). 히트 포즈의 법선을 **받아 놓고 버리고 있었다**:
벽을 탭하면 각도가 공짜로 오고, 훑기는 3m·120표본에 **0.240°**(손계산과 일치). `WARMUP_MS`
타이머는 **준비도**로 바뀌어, 링이 차오르고 초록에 진동하며 진단 대신 **다음 동작 한 줄**만
뜬다. 근거 `evidence/2026-08-07/xr-calibration.md`. 마커는 정밀 정합용으로 남긴다.

**다음 한 걸음** — ① **폰 실기 AR (0단계 · 아직 안 함)** — `앵커 붙음` · `겨냥 높이↔바닥` ·
제자리 유지 · **그 방에서 벽이 평면으로 잡히는가**(우선순위를 이게 정한다) ② **3250×3250 ·
벽 250mm 맵** ③ `zones[]` 진입 판정. 남은 관문 — **`mapToLab`** · XR 원점 리셋 · 터틀봇 GLB.
