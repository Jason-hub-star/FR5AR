...
`AR/xr.html` (`fr5ar.vercel.app/xr.html`) — **모드 넷 · 바텀시트**(2026-08-06 개편).
평소(1:2.5) · **답사(1:1 · 걸어 들어간다)** · 촬영(1:1+두 모서리 보정) · 화면(XR 없이 · PC 확인용).
고를 것은 **모드와 맵 둘**뿐이다 — 팔 대수·애니메이션은 맵이 정한다. 시작 화면에서 **맵이
천천히 돌며** 미리 보인다(`updateCutaway` 를 이 화면이 처음 쓴다). 근거 `evidence/2026-08-06/`.
마커 경로는 **정밀 정합용으로 남긴다** — mm 정렬과 원점 고정은 여전히 마커다.

**다음 한 걸음** — ① **폰에서 실기 AR** — 로그의 `앵커 붙음` · `겨냥 높이↔바닥` 차이 · 걸어갔다
와서 제자리인지 ② **3250×3250 · 벽 250mm 맵**을 만들어야 촬영 모드가 실제 맵과 맞는다
③ `zones[]` 를 화면에 그리고 mock pose 로 진입 판정 (`SHARED-CORE.md` §zones)
