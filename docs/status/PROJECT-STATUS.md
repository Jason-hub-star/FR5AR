...
**이번 세션의 교훈** — 결함 12개 **전부 게이트·빌드를 통과한 뒤** 나왔고, 그중 3개는
**배포본을 실제로 클릭해서야** 나왔다. `grep`·`build`·합성 이벤트는 화면이 거짓말하는 것을
못 잡는다. 대시보드 마감은 **실제 클릭·드래그**까지 가야 한다.

**다음 갈래 셋.** ①폰 확인(가장 싸다 — `npm run dev:dash`, 좁은 창에서 패널 접힘과 터치 문턱)
②저장을 팀 공유로(이관 H, `Shared/data/config/` 슬롯) ③고정 카메라 1단계(호모그래피 4점
보정 — **환경과 무관해서 지금 지어도 안 버려진다.** 배치안을 실공간에 띄우는 것과
실공간을 화면에 되받는 것 둘 다 이 보정을 쓴다).
