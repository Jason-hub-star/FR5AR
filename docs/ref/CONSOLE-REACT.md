...
| 4 | 로봇 조작·조종권·상태 (F1·F2·F5) | 버튼·상태 표시가 많다 |

1~3이 새 목표로 생긴 것이고, 4는 원래 있던 역할이다. **한 폴더에 둔다** — 둘 다 PC가 열고
같은 서버 상태를 본다. 탭으로 나눈다.
...
| 서버 상태 (관절·모드·조종권) | WebSocket → 한 곳에서 받아 내려준다 (`API-CONTRACT.md`) |
