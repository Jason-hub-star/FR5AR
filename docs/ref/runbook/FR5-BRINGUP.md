...
2. 로봇 ↔ 우분투 **랜선** 연결 — 우분투는 `enp3s0`(고정 `192.168.57.10/24`),
   로봇은 **컨트롤러 1번 포트**. ⚠ **비상정지 박스 쪽 랜 단자에 꽂으면 로봇이
   `192.168.58.2` 로 보인다** (2026-08-04 실측) — 주소가 다르면 여기부터 본다
3. 우분투 전원 · 팀 와이파이 접속
...
| 증상 | 원인·조치 |
|---|---|
| ping 실패 | **`ip -brief addr show enp3s0` 부터 본다.** 주소가 비었으면 프로필이 DHCP 로 되밀린 것이다 (2026-08-04 실측) → `sudo nmcli connection up netplan-enp3s0`. 링크만 보고 판단하지 않는다 — 1000Mb/s 로 잡혀도 주소가 없으면 못 간다 |
