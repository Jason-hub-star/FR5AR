...
### 카메라 — 라즈베리파이가 세 번째 관문이다 (D48)

**카메라의 주인은 파이다.** 로봇 옆 FR5 브리지가 아니라, 카메라를 물고 전원을 대는 파이 위에
작은 서버를 올린다. "장치 하나에 관문 하나" — tb-bridge·fr5-bridge 와 같은 패턴이다.

```
GET  /api/camera/info          { model, serial, w, h, fps, calibId }
...
- **파이는 로봇에 명령하지 않는다.** 찍고·시각 찍고·저장할 뿐이다. 실행 여부는 언제나
  FR5 브리지의 안전 게이트가 정한다 (`Vision/AGENTS.md` §제안만 만든다)
- **원본 깊이를 실시간으로 흘리지 않는다.** 파이가 로컬에 먼저 쓰고 나중에 동기화한다.
  실시간 경로는 압축 컬러(화면 확인·정책 입력)뿐이다 — 그래서 **무선으로 충분하다**
- **타임스탬프는 도착 시각이 아니라 촬영 시각**을 파이가 직접 찍는다. 도착 시각으로 찍으면
