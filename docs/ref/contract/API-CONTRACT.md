...
## 프로그램 슬롯 — 이 문서가 아니라 `PROGRAM-CONTRACT.md` (2026-08-06 신설)

지점을 순서로 엮어 `draft→approve→step` 하는 계약은
[`PROGRAM-CONTRACT.md`](PROGRAM-CONTRACT.md) 에 있다. 수명주기와 소비자가 상태·명령과 달라
따로 산다 (카메라·제안을 뺀 것과 같은 이유).

**§명령의 허용목록은 그대로다** — 슬롯 실행은 `moveJ` 를 통해서만 하드웨어에 닿는다.
§이동 지점의 "참조하는 슬롯이 있으면 보관만" 이 가리키는 슬롯이 그것이다.
