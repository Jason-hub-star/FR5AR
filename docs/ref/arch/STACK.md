...
| 뎁스카메라 | Intel RealSense D435 | — | 손끝 위 80mm · 툴 축 옆 82mm (실측) — §뎁스카메라 |
...
⚠ 남은 것은 정확도가 아니라 **기준점**이다. 135 는 **핑거 끝**이고, 물건을 잡는 지점은
핑거 절반쯤 안쪽이다 (**≈122.5mm**). 집기 좌표를 쓰는 순간 이 12.5mm 가 그대로 오차가 된다 —
`toolCoordId` 를 새로 뜰 때 "끝" 인지 "파지 중심" 인지부터 정한다.
