...
### 모방학습 시연 (신규 요구 · 2026-08-03 팀 회의)

그리퍼에 뎁스카메라를 달고 사람 시연을 관찰해 재현한다. **PRD 의 "자율 작업 계획 범위 밖"과
충돌하지 않는 선을 여기서 긋는다** (감사 P0):

> 시연 1건 → 궤적 1건 재생. 새 물체·새 배치로 **일반화하지 않는다.**

- 시연 데이터(영상 포함)는 `~/fr5-data/demos/` — 용량 상한과 보존기간을 두고, 촬영 착수
  **전에** 정한다. 사람이 찍히므로 로봇 상태와 같은 공개 수준으로 두지 않는다 (감사 P1)
- 재현 실행은 슬롯과 같은 관문을 탄다 — `approve` 후 `run`, 매 스텝 안전 게이트.
  사람이 검토하지 않은 궤적이 5° 상한 안에서 연속으로 나가는 경로를 열지 않는다 (감사 P2)
