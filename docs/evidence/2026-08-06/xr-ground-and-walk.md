...
## `/감사` 2차 — 새로 쓴 코드 (2026-08-06)

바텀시트 개편·미리보기·`place.js`·검증기는 1차 감사 뒤에 쓴 것이라 아직 아무도 안 봤다.

| | 판정 | 발견 | 근거 |
|---|---|---|---|
| A1 | **P1 · 고침** | **팔 받는 내내 검은 화면.** `renderer.setAnimationLoop` 이 `await mountArms` **뒤**에 있어, 6MB×3 을 받는 동안 캔버스가 한 번도 안 그려졌다 — U3 를 절반만 고친 셈이었다 | 3Mbps 로 조여 실렌더 촬영 → 검은 화면 + 안내 한 줄뿐. 고친 뒤 같은 조건에서 **60fps 로 방이 즉시** 뜬다 |
| A2 | P2 · 고침 | 미리보기가 **매 프레임 `getBoundingClientRect`** 를 불러 레이아웃을 강제로 다시 계산했다 | 렌더 루프 안이라 폰에서 비싸다. `ResizeObserver` 로 바뀔 때만 잰다 |
| A3 | **기각** | WebGL 컨텍스트 누수를 의심했다 (미리보기↔세션을 오갈 때마다 새로 만든다) | **7왕복 실측 — 캔버스 1개 유지 · 콘솔 에러 0.** `stage.dispose()` 가 제대로 걷어낸다. 버그 아님 |

A1 을 고치면서 히트테스트도 팔보다 먼저 연다 — 받는 동안에도 조준 링이 산다. 루프가
아직 없는 소스를 물지 않게 `hits ? … : []` 가드를 같이 넣었다.

**회귀 검사를 박았다** — 캔버스 가운데 픽셀을 `readPixels` 로 읽어 팔이 0대인 시점에 이미
밝은지 본다 (`가운데 픽셀 합 587 · 그 시점 팔 0대`). 이제 44/44.
