...
**"화면이 안 채운다" 는 내 진단이 틀렸다.** 두 건은 뿌리가 서로 달랐고, 화면은 둘 다 멀쩡했다.

## ① 로그 백로그 — 버퍼가 빈 게 아니라 **넣은 적이 없었다**

`log_stream.py` 의 500줄 deque 도, `subscribe()` 의 백로그 재생도 계약대로 있다.
빠진 것은 **넣는 쪽**이었다 — `logs.emit` 호출은 estop·매핑·맵저장처럼 **사건이 있을 때만**
있고, 기동 자체는 아무것도 안 남긴다. 그래서 갓 뜬 브리지의 첫 접속은 늘 0줄이다.

검증이 까다로워서가 아니라 **사람에게도 나쁘다** — 로그 패널이 빈 채로 열리면 보는 사람이
*"로그가 안 오나"* 와 *"아직 아무 일도 없었나"* 를 못 가른다. 기동 한 줄을 남긴다:

```
tb-bridge 기동 · adapter=real · 로봇 tb3_1, tb3_2
```

`adapter=` 를 함께 넣은 것은 덤이 아니다 — 나중에 기록을 되짚을 때 **그 세션이 mock 이었는지
실기였는지**가 로그만으로 판별된다 (SR_24 · 목업을 실측으로 오인하는 사고가 제일 비싸다).

## ② 기록 목록 — 검증이 **빈 상태 행을 데이터로 세고** 먼저 읽었다

`RunList` 는 탭을 열 때마다 새로 마운트돼 `reload()` 를 부른다. 정상이다.
문제는 검증의 대기 조건이었다:
...
빈 상태 행(`기록이 없어요`)도 `tbody tr` 이라 **첫 렌더에서 이미 참**이다. 그래서 fetch 가
돌아오기 전에 통과했고, 바로 다음 줄이 `.badge` 를 못 찾았다. 그 다음다음 줄의
`fetch('/api/runs')` 는 PASS 했으니 **API 도 화면도 맞고 읽는 시점만 일렀다.**

고친 쪽은 둘 다다 — 빈 행에 `runs-empty` 표식을 붙이고(빈 상태와 데이터가 DOM 에서 구별돼야
한다), 검증은 `:not(.runs-empty)` 를 기다린다.

## 결과

`tb-bridge-verify` **22/22 전체 PASS**(맥·mock) · `tb-real-verify` **6/6 PASS**(우분투 배포본).
