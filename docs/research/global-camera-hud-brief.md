...
| 글로벌 카메라 내부 파라미터 | **틀렸다 (2026-08-06)** | 저장값 `fx 2106 · HFOV 62.6°` 는 근거 없음. 태그 역산과 ChArUco 재촬영이 독립으로 **fx ≈ 1750 · HFOV ≈ 72°** 를 가리킨다 (`evidence/2026-08-06/global-cam-intrinsics-invalid.md`). **아직 안 고쳤다** — 게이트가 막았다 |
| 글로벌 카메라 외부 파라미터 | **없음** | `global-cam.json`에 `labToCam` 없음 |
...
| 1.5 | **내부 파라미터 재측정** (`intrinsics.py`) | ⚠ 신규. 저장값이 틀렸다 — RMS ≤0.5px **그리고** 앞/뒤 fx 차이 ≤5% |
| 2 | `global-cam.json` 에 `labToCam` 실측 | `scripts/map/extrinsics.py` 산출, **RMS ≤ 2.0px** (`MAX_EXTRINSIC_RMS`). 0.5px 은 내부 파라미터 상한이지 외부가 아니다. **찍기 전 `cam-lock.sh`** — 설정이 조용히 되돌아간다 (D64) |
...
- **✅ 해상도 불일치 — 스케일 하나로 정확히 닫힌다 (2026-08-06 실측)**: 1920×1080 은
  ...
  **배율 0.7502 · 회전 0.01° · 이동 0.2px** (크롭 가설이면 1.0000 이 나와야 한다).
  ...
  → **재측정하지 않는다.** 2560×1440 내부 파라미터(35/35 코너 · RMS 0.4107px)를 그대로 두고
  `fx·fy·cx·cy × (스트림폭/보정폭)` 만 한 함수에 둔다. **`dist` 는 정규화 좌표라 안 바뀐다.**
  1080p 로 다시 재면 코너 검출이 28/35 로 떨어진다 (`cam-lock.sh` 실측 주석).
- **❌ 왜곡을 핀홀로는 못 그린다 — 실측 확정 (2026-08-06 · 손계산)**: `dist` 를 실제로 적용해
  ...
  **어긋남 ≤1px 가 유지되는 건 반경 321px 안뿐이다** — 화면 대각 반경 1469px 의 **22%**.
  즉 three.js `PerspectiveCamera` 만으로는 **화면 중앙 5분의 1 밖은 못 쓴다.**
  §8 의 "1px 이내" 는 그 반경 안에서만 성립한다. 밖까지 그리려면 왜곡 보정이 필수다.
  (참고: `fx 2106 / fy 2059` 로 이방성 2.2% — 폰 렌즈치고 크다. 사진 20장이 적었을 수 있다.)

  ⚠⚠ **2026-08-06 정정 — 위 숫자는 무효다.** 저 계산은 지금 저장된 `dist` 를 참으로 가정했는데,
  그 계수 자체가 초점거리와 맞바꿔진 허상으로 판정됐다 (`fx` 가 1351~2351 로 74% 흔들렸다).
  실제 왜곡은 새 캘리브레이션 뒤에 다시 잰다. **공간 HUD 착수 전 선행 조건이 하나 늘었다 —
  내부 파라미터 재측정.**
