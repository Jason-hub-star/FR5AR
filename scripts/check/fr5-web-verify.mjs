...
try {
  ...
  check('상시 안전 바 8항목',
    (await p.eval(`document.querySelectorAll('.safeitem').length`)) === 8);
  ...
  check('출처 배지 mock (사칭 없음)',
    (await p.eval(`document.querySelector('.source').dataset.src`)) === 'mock');
  check('펌웨어 실측 문자열 표시',
    !!(await p.waitFor(`document.body.textContent.includes('FR_CTRL_FV2.010.12')`, { timeoutMs: 5000 })));

  // 4. 값이 흐른다 — 관절 표가 스트림을 따라 변한다
  const j1a = await p.waitFor(`document.querySelector('.joints td')?.textContent !== '—' && document.querySelector('.joints td')?.textContent`, { timeoutMs: 5000 });
  await new Promise((r) => setTimeout(r, 1200));
  const j1b = await p.eval(`document.querySelector('.joints td')?.textContent`);
  check('관절값 스트림 갱신 (WS)', !!j1a && j1a !== j1b, `${j1a} → ${j1b}`);
  check('TCP 6행 값 표시',
    (await p.eval(`[...document.querySelectorAll('.tcp td')].filter(td => td.textContent !== '—').length`)) === 6);

  // 5. 3D 쌍둥이 — URDF 실로딩 + 픽셀이 실제로 움직인다
  check('URDF+그리퍼 로딩 완료 깃발',
    !!(await p.waitFor(`document.querySelector('.twin')?.dataset.ready === '1'`, { timeoutMs: 20000 })));
  const rect = await p.rect('.twin canvas');
  ...
  const reconn = await p.waitFor(`parseInt(document.querySelector('.ws-reconnects')?.textContent) >= 1 && document.querySelector('.ws-reconnects').textContent`, { timeoutMs: 15000 });
  ...
  // 7. 해제 → 미연결 스냅샷으로 복귀
  await p.eval(`[...document.querySelectorAll('.diag button')].find(b => b.textContent.includes('연결 해제')).click()`);
  check('disconnect → DISCONNECTED + 값 비움',
    !!(await p.waitFor(`document.querySelector('.safetybar').textContent.includes('DISCONNECTED') && document.querySelector('.joints td')?.textContent === '—'`, { timeoutMs: 5000 })));
} catch (e) {
