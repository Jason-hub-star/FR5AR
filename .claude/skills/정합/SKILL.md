---
name: 정합
description: 값·용어·결정이 바뀌었을 때 뿌리 SSOT부터 고쳐 모든 문서·스크립트로 전파하는 스킬. "정합 맞춰줘" "이 값 바꼈어" "영향 범위 알려줘" 에 발동. 고치기 전 영향 범위 매핑을 먼저 한다.
user_invocable: true
tags: [domain, propagate, write]
trigger: 기준값·용어·결정이 바뀔 때 · cross-cutting 변경 전 영향 범위 파악
version: 1
---
...
`/정합` 이 흡수한 것 — 영향 범위 매핑(`references/영향범위-매핑.md`).
**route·스키마·환경변수·폴더 경로처럼 여러 곳을 건드리는 변경**은 그 절차로 파급을 먼저 적고,
그 목록대로 이 문서의 전파를 돈다. 순서를 바꾸면 빠뜨린 참조가 남는다.

### 함께 보는 것

- `references/영향범위-매핑.md` — 파급을 먼저 그리는 절차
- `references/impact-cases.md` — 영향 범위 실제 사례
