# TurtleBot — 터틀봇 관제 (웹앱 · tb-bridge **mock 으로 전 사이클 동작** — 실기는 P4)

**계약이 먼저다** — `docs/ref/TB-CONTRACT.md` 를 고치고 코드를 짠다.
순서(D29~D31): 계약 → `/감사` → UI 목업 → 맥 mock 구현 → 우분투 실기.

- 웹앱(Vite+React, :5175)과 브리지(FastAPI, :5055)가 **이 폴더에 수직 완결** —
  우분투 PC 에서 `git pull` 하나로 배포된다. Vercel 에 올리지 않는다 (D29)
- **ROS 는 `bridge/ros_adapter/` 안에서만.** mock↔real 파일 교체가 환경 전환의 전부 —
  맥에서 개발하고 우분투에서 실행한다 (D30)
- 화면은 `fetch` 금지 — `src/data/datasource/` 경유 (Dashboard 규칙 미러)
- `Dashboard/` 와 상호 import 금지. FR5 `bridge/` 와 무의존 — 로봇마다 관문 하나
...
| `bridge/ros_adapter/` | ROS 유일 경계 — mock 동작 · real 은 P4 (repo2 대조 후) |
