# TurtleBot — 터틀봇 관제 (웹앱 · tb-bridge · **P0~P4 완료, 실기 접속은 막혀 있다**)

**계약이 먼저다** — `docs/ref/contract/TB-CONTRACT.md` 를 고치고 코드를 짠다 (D29~D31).
**포트는 5056** — 같은 PC(`ej@192.168.30.240`)의 5055 는 FR5 것이다 (D80).
정본은 `bridge/config.yaml` 의 `port` 한 줄 · 기동 스크립트가 읽는다. 남은 건 로봇 전원뿐.
